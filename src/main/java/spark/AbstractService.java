 ...
 * If a user wants multiple 'Sparks' in his application the method {@link AbstractService#ignite()} should be statically
...
public abstract class AbstractService<T> extends Routable {
     ...
     */
    public synchronized T webSocketIdleTimeoutMillis(long timeoutMillis) {
        if (initialized) {
            throwBeforeRouteMappingException();
        }
        if (isRunningFromServlet()) {
     ...
     */
    public synchronized <E extends Exception> void exception(Class<E> exceptionClass, ExceptionHandler<? super E> handler) {
        // wrap
        ExceptionHandlerImpl<E> wrapper = new ExceptionHandlerImpl<E>(exceptionClass) {
            @Override
     ...
     */
    public synchronized T trustForwardHeaders() {
        if (initialized) {
            throwBeforeRouteMappingException();
        }
     ...
     * More info at https://www.eclipse.org/jetty/javadoc/current/org/eclipse/jetty/server/ForwardedRequestCustomizer.html
     */
    public synchronized T untrustForwardHeaders() {
        if (initialized) {
            throwBeforeRouteMappingException();
        }
